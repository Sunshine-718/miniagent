from src.tools import search_memory
import os
...
@@@ Action
<工具函数名 (准确字符串，独占一行)>
(例如: `search_memory` 或 特殊指令: `[REFRESH]`, `[QUIT]`, `[CLEAR]`)
只有三个特殊指令，不允许创造其它的特殊指令。
...
- **时间**: {datetime.now()}
- **路径**: {os.getcwd()}
- **日志**: {len(os.listdir('./logs'))} active logs.
"""
...
1. **单步执行原则 (ONE STEP LIMIT)**：
   ...
   - **python_repl**: 可以用python_repl合并多个Action以节省token。
