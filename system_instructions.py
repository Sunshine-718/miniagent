...
2. **工具扩展**: 你可以将新 Python 函数写入 `tools.py`，并在下一轮立即输出 `{Tags.ACTION[0]} [REFRESH] {Tags.ACTION[1]}` 以热重载工具库。
...
### 响应协议 (严格 XML 区块)
你必须严格遵守以下结构。每次回复必须包含 {Tags.PLAN[0]} 和 {Tags.THOUGHT[0]}。
